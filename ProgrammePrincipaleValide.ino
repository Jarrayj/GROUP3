...
// Rafraîchissement LCD : on ne réécrit que sur transition pour ne pas
// ralentir la boucle de suivi. tDernierAffichage sert uniquement pour
// la distance à l'arrêt final.
PhaseGlobale  dernierePhaseAffichee  = (PhaseGlobale)-1;
ModeTunnel    dernierModeTunnelAff   = (ModeTunnel)-1;
EtatCouleur   dernierEtatCouleurAff  = (EtatCouleur)-1;
int           dernierObstacleAff     = -1;
unsigned long tDernierAffichage      = 0;
...
void mettreAJourAfficheur() {
  // ── Attente départ ───────────────────────────────────────────────
  if (phaseGlobale == PHASE_ATTENTE_3S) {
    if (dernierePhaseAffichee != PHASE_ATTENTE_3S) {
      afficherLCD(" DEPART DANS", "      3s...", 255, 255, 255);
      dernierePhaseAffichee = PHASE_ATTENTE_3S;
    }
    return;
  }

  // ── Tunnel ───────────────────────────────────────────────────────
  if (phaseGlobale == PHASE_TUNNEL) {
    if (modeTunnel == TUNNEL_SUIVI &&
        (dernierePhaseAffichee != PHASE_TUNNEL || dernierModeTunnelAff != TUNNEL_SUIVI)) {
      afficherLCD("  SUIVI LIGNE", "", 0, 200, 0);
      dernierePhaseAffichee = PHASE_TUNNEL;
      dernierModeTunnelAff  = TUNNEL_SUIVI;
    } else if (modeTunnel == TUNNEL_NAVIGATION &&
               (dernierePhaseAffichee != PHASE_TUNNEL || dernierModeTunnelAff != TUNNEL_NAVIGATION)) {
      afficherLCD("    TUNNEL", "  Navigation", 0, 0, 255);
      dernierePhaseAffichee = PHASE_TUNNEL;
      dernierModeTunnelAff  = TUNNEL_NAVIGATION;
    }
    return;
  }

  // ── Évitement ────────────────────────────────────────────────────
  if (phaseGlobale == PHASE_EVITEMENT) {
    if (dernierePhaseAffichee != PHASE_EVITEMENT || dernierObstacleAff != obstacleIndex) {
      char ligne2[17];
      if (obstacleIndex == 0)      snprintf(ligne2, sizeof(ligne2), "  Obstacle 1/2");
      else if (obstacleIndex == 1) snprintf(ligne2, sizeof(ligne2), "  Obstacle 2/2");
      else                         snprintf(ligne2, sizeof(ligne2), "  Fin evitement");
      afficherLCD("  EVITEMENT", ligne2, 255, 100, 0);
      dernierePhaseAffichee = PHASE_EVITEMENT;
      dernierObstacleAff    = obstacleIndex;
    }
    return;
  }

  // ── Couleur ──────────────────────────────────────────────────────
  if (phaseGlobale == PHASE_COULEUR) {
    if (dernierePhaseAffichee != PHASE_COULEUR || dernierEtatCouleurAff != etatCouleur) {
      if (etatCouleur == SEQUENCE_RAMPE) {
        afficherLCD("   COULEUR", "    RAMPE", 180, 0, 180);
      } else {
        afficherLCD("   COULEUR", "  Suivi ligne", 180, 0, 180);
      }
      dernierePhaseAffichee = PHASE_COULEUR;
      dernierEtatCouleurAff = etatCouleur;
    }
    return;
  }

  // ── Arrêt final : distance en continu toutes les 300ms ───────────
  if (phaseGlobale == PHASE_ARRET_FINAL) {
    if (dernierePhaseAffichee != PHASE_ARRET_FINAL) {
      lcd.setRGB(255, 0, 0);
      lcd.clear();
      lcd.setCursor(0, 0); lcd.print("ARRET - Dist:");
      dernierePhaseAffichee = PHASE_ARRET_FINAL;
      tDernierAffichage     = 0;
    }
    if (millis() - tDernierAffichage > 300) {
...
void lancerBalle() {
  ...
  afficherLCD("  LANCEMENT", "  BALLE !", 255, 200, 0);
  demarrerLanceur();
  delay(DUREE_LANCEMENT_MS);
  arreterLanceur();
  afficherLCD("  BALLE", "  LANCEE !", 255, 200, 0);
        ...
        case TUNNEL_SUIVI: {
          ...
          if (ligneVue) {
            ...
            if (protectionActive) {
              ...
              avancer(70, 2); delay(570); stopper();
